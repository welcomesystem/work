...
public class AddDB {
	...
	public int addStuAccount(InfoData student) {//添加学生账号
    	PreparedStatement pStmt = null; 
    	int count=0;
    	try {
    		pStmt = con.prepareStatement("insert student into STU_ID,PAS_ST valuse (?,?)");
    		count = pStmt.executeUpdate();
    		pStmt.close();
    	}
    	catch(Exception e) {
	...
	public int addTea(TeaData teacher) { //添加教职工,返回成功，0失败
        ...
        try {
    		...
    		pStmt.setInt(3, teacher.getTea_po());//添加教职工职位
	...
	public int addTeaAccount(TeaData teacher) {//添加教师账号
		PreparedStatement pStmt = null; 
		int count=0;
		try {
			con=DBConnection.getConnection();
			pStmt = con.prepareStatement("insert into teacher (TEA_ID,PAS_TE,TEA_ROOT) values (?,?,?)");
			pStmt.setInt(1,teacher.getTea_id());
			pStmt.setInt(2,teacher.getTea_id());
			pStmt.setInt(3,teacher.getTea_po());
